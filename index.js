    ...
    dbmgr.UserRecord.find({mutedUntil: {$gt: 0, $lte: moment().unix()}}, function (err, docs) {
        ...
        for (var doc of docs) {
            ...
            for (var guild of bot.guilds) {
                ...
                if (!member) {
                    processError("UNMUTE GM QUERY", "No results.\nUser ID: " + doc.userid + "\nLast known username: " + doc.username);
                    modLog("**[MUTE LIFT FAIL]** for user _" + doc.username + " (" + doc.userid + ")_. Log posted in #" + config.botLogChannel + ".");
                }
